...
export function isValidUserName(username) {
    ...
    if (username.length < 8) {
        validation = { valid: false, message: 'Имя пользователя должно содержать не менее 8 символов.' };
...
export function isValidEmail(email) {
    ...
    if (email.length < 4) {
        validation = { valid: false, message: 'Email должен содержать не менее 4 символов.' };
