...
function sendCookieToServer(loggedInCookie) {
    fetch('/api/login', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ loggedInCookie }),
    })
        .then((response) => {
            if (response.ok) {
                console.log('Cookie успешно отправлен на сервер');
            } else {
                console.error('Ошибка при отправке cookie на сервер');
            }
        })
        .catch((error) => {
...
function loginUser(username, password) {
    ...
    .then((data) => {
        if (data.success) {
            ...
            sendCookieToServer(loggedInCookie);
        } else {
